    ...
    <table class="min-w-full divide-y divide-gray-300">
      ...
      <tbody class="divide-y divide-gray-200 bg-white">
        <tr v-for="count in testRunCounts" :key="count.testID" :set="totalRunCount=count.runCounts[RunResult.PASS] + count.runCounts[RunResult.FAIL] + count.runCounts[RunResult.ERROR] + count.runCounts[RunResult.UNKNOWN]">
          ...
          <td class="whitespace-nowrap px-2 py-2 text-sm text-gray-500">{{ totalRunCount }}</td>
          <td class="whitespace-nowrap px-2 py-2 text-sm text-green-500">{{ count.runCounts[RunResult.PASS] }}</td>
          <td class="whitespace-nowrap px-2 py-2 text-sm text-red-500">{{ count.runCounts[RunResult.FAIL] }}</td>
          <td class="whitespace-nowrap px-2 py-2 text-sm text-pink-500">{{ count.runCounts[RunResult.ERROR] }}</td>
          <td class="whitespace-nowrap py-2 pl-3 pr-4 text-sm text-gray-500 sm:pr-6">{{ count.runCounts[RunResult.UNKNOWN] }}</td>
        </tr>
      </tbody>
    </table>
  </div>
